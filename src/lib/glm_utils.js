...
function Material(name) {
  ...
  this.triangles = new Array(); /* array of triangle indices */
  if (name == "default") {
    this.parameter.set("Ka", [1.0, 1.0, 1.0]);
    this.parameter.set("Ks", [0.5, 0.5, 0.5]);
    this.parameter.set("Kd", [0.8, 0.8, 0.8]);
    this.parameter.set("d", 1.0);
...
function FacetNormals(mesh) {
  ...
  for (var i = 1; i <= mesh.nface; i++) {
    ...
    u[0] =
      mesh.vert[mesh.face[i].vert[1]].x - mesh.vert[mesh.face[i].vert[0]].x;
    u[1] =
      mesh.vert[mesh.face[i].vert[1]].y - mesh.vert[mesh.face[i].vert[0]].y;
    u[2] =
      mesh.vert[mesh.face[i].vert[1]].z - mesh.vert[mesh.face[i].vert[0]].z;

    v[0] =
      mesh.vert[mesh.face[i].vert[2]].x - mesh.vert[mesh.face[i].vert[0] + 0].x;
    v[1] =
      mesh.vert[mesh.face[i].vert[2]].y - mesh.vert[mesh.face[i].vert[0]].y;
    v[2] =
      mesh.vert[mesh.face[i].vert[2]].z - mesh.vert[mesh.face[i].vert[0]].z;
...
function Unitize(mesh) {
  ...
  for (var i = 2; i < mesh.vert.length; i++) {
    if (maxx < mesh.vert[i].x) maxx = mesh.vert[i].x;
    if (minx > mesh.vert[i].x) minx = mesh.vert[i].x;

    if (maxy < mesh.vert[i].y) maxy = mesh.vert[i].y;
    if (miny > mesh.vert[i].y) miny = mesh.vert[i].y;

    if (maxz < mesh.vert[i].z) maxz = mesh.vert[i].z;
    if (minz > mesh.vert[i].z) minz = mesh.vert[i].z;
  }
  ...
  for (var i = 1; i < members.length; i++) {
    ...
    for (var j = 0; j < members[i].indexFace.length; j++) {
      if (node.averaged[j]) {
        ...
        numnormals++;
        mesh.normal[numnormals] =
          mesh.facetnorms[mesh.face[members[i].indexFace[j]].normalFaceIndex];
        for (h = 0; h < mesh.face[members[i].indexFace[j]].n_v_e; h++)
          if (mesh.face[members[i].indexFace[j]].vert[h] == i)
            mesh.face[members[i].indexFace[j]].normalVertexIndex[h] =
              numnormals;
      }
    }
  }
  ...
  for (j = 0; j < lines.length; j++) {
    ...
    switch (buf[0]) {
      ...
      case "mtllib":
        ...
        break;
      case "usemtl":
        if (!buf[1].includes("null"))
          material = AddElement(mesh.materials, buf[1], Material);
        break;
      case "g": // group
  ...
  for (j = 0; j < lines.length; j++) {
    ...
    switch (buf[0]) {
      ...
      case "newmtl":
        ...
        while (
          j < lines.length &&
          (buf = lines[j].trimRight().trimLeft().split(/\s+/)).length > 1
        ) {
          if (material) {
            var v;
            switch (buf[0]) {
